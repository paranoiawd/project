...
문헌 조사의 결과이고, **그 뒤 1·2순위 후보를 실제로 구현해 측정한 결과까지 포함한다.**
(a) 이 문제의 학술적 좌표를 찍고, (b) 분야별 대표 방법을 HANDOFF §6(실패 실험 ~70개)과
대조해 걸러내고, (c) 후보를 랭킹한 뒤, (d) 상위 두 개를 실행해 닫았다.

**먼저 읽을 것: §3.5.** 1·2순위(스카우트 투어 최적화, rollout)는 **종결**이다. 오프라인
모델은 +0.42의 헤드룸을 가리켰지만 시뮬레이터 안에는 없었고, 그걸 증명한 진단
(`./bench <seed> 16 terrain`)이 이 세션의 가장 재사용 가치 높은 산출물이다.
스케줄러 자체는 **변경되지 않았다** (300시드에서 0.000 ± 0.000 차이로 검증).
...
## 3.5. 결과 (같은 세션에서 A·B를 실제로 구현해 측정함) — **A·B 종결**

아래 §4의 랭킹은 조사 시점의 것이고, 1·2순위는 그 뒤 구현·측정되어 **닫혔다.**
상세 수치는 HANDOFF §3a와 §6에 있다. 요약:

1. **§5 1단계(A의 상한 체크)는 "진행"을 가리켰다.** `bench/scoutbound`로 재면, 정보와
   페이싱을 동일하게 맞춘 상태에서 투어 최적화는 **+0.418 ± 0.024 기대 발견, 100/100
   시드 승리**. greedy 베이스라인은 허수아비가 아니도록 10개 변형(locality offset 5종 ×
   밴드 유무) 중 최고를 썼다. 모델은 실제 발견 수를 0.1 이내로 재현했다(13.54 vs 13.44).
2. **그런데 그 헤드룸은 시뮬레이터에 존재하지 않는다.** 새 진단
   `./bench <seed> 16 terrain`(지형만 공개, 태스크는 계속 숨김)으로 지도를 공짜로 다 줘도
   **발견은 +0.000 ± 0.083**이다. 완료만 +0.190 ± 0.080 오르는데, 그건 워커가 아는 땅 위로
   경로를 잡아서지 관측이 달라져서가 아니다.
3. **B(rollout)는 5개 설정 전부 ±0**, 그리고 후보 수를 늘리면 오히려 약해졌다(+0.143 →
   +0.020). 지도를 공개한 상태에서도 ±0. A-lite(도착 시점 가치)도 정확히 ±0.

**물리적 이유**: 드론 2대가 2000틱에 살 수 있는 건 약 1.25회 스윕이고, 한 걸음은 어디서
딛든 새 셀 5개를 본다. **스윕의 순서를 바꿔도 관측의 총 cell-tick은 늘지 않는다.**
HANDOFF의 "정보는 살 수 없다"에 "경로로 만들어낼 수도 없다"가 추가된 셈이다.

**방법론 교훈 (가장 값비싼 것)**: `scoutbound` 모델은 베이스라인 *수준*은 0.1 이내로
맞히면서 *개입에 대한 반응*은 지도 지식의 가치를 한 태스크만큼 과대평가했다.
**모델은 재현하려는 베이스라인이 아니라 하려는 개입에 대해 캘리브레이션해야 한다.**
...
### A. 스카우트 투어의 전역 최적화 — 시간가변 보상 오리엔티어링 (~~1순위~~ **종결, §3.5**)
...
### B. 스카우트 타겟 선택에 rollout — A의 저비용 파일럿 (~~2순위~~ **종결, §3.5**)
...
### C. MSA 시나리오 합의 — 워커의 "가는 김에" 극대화 (**현 1순위**)
...
### E. 조인트 파라미터 최적화 — algorithm configuration (**현 3순위**, 야간 배치)
...
## 5. 제안 실행 순서 (다음 세션) — A·B 종결 후 갱신

1. ~~A의 상한 체크~~ → **완료**. 갭은 오프라인에 있었으나 시뮬레이터에는 없었다(§3.5).
2. ~~B(rollout)~~ → **완료, ±0**. A로 확전할 근거 없음.
3. **C(MSA)** 가 남은 최상위 후보다. 다만 사전 확률은 여전히 낮고(§4.C), 배관 작업이 크다.
   시작 전에 C에도 A와 같은 값싼 상한 체크를 먼저 만들 것 — **이번 세션이 증명한 것은
   상한 체크의 가치가 아니라, 상한 체크가 개입이 아니라 베이스라인에 캘리브레이션되면
   틀린 답을 준다는 것이다.** C의 상한 체크는 반드시 시뮬레이터 안에서 (`terrain` 처럼
   진짜 개입으로) 재야 한다.
4. **D**는 어느 경로든 끼워넣을 수 있는 반나절짜리.
5. **E**는 세션 종료 시 야간 배치로 걸어두는 용도. A·B가 닫힌 지금 상대적 순위가 올랐다.

### 이 세션이 남긴 도구 (재사용할 것)
* `./bench <seed> 16 terrain` — 지형만 공개. "지도를 아는 것"과 "일이 어디 있는지 아는 것"을
  분리한다. 어떤 라우팅 아이디어든 이걸로 천장을 먼저 재라.
* `bench/scoutbound.cpp` + `bench/scoutcmp.py` — 관측 기록을 기대 발견 수로 환산하고,
  드론 투어를 greedy/SA 두 방식으로 재계획해 비교한다.
